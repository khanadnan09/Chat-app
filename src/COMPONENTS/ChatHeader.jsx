  ...
  return (
    <div className="col-12 top_left_header d-flex justify-content-between" style={{ padding: " 7.5px 16px" }}>
      <div className="user_wraper d-flex align-items-center">
        <ion-icon name="arrow-back-outline" className="hide_show md hydrated" onclick="closeChat()" style={{ position: "relative", left: " -10px", display: "none", cursor: "pointer" }} role="img" aria-label="arrow back outline"></ion-icon>
        <img src="https://mywhatsappwebclone.netlify.app/IMAGES/ps8.jpg" alt="" className="uesr_top_left_header" style={{ cursor: "pointer" }} />
        <div className="text_info_wrapper ps-2">
          <span className="user_name">{roomName}</span>
          <span className="user_sm_msg position-relative" style={{ top: "-5px" }}>last seen {lastMsg.length ? new Date(lastMsg[0].timeStamp?.seconds*1000).toLocaleString():"Loading..." }</span>
        </div>
      </div>
